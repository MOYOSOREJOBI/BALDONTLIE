...
export default function OddsPage() {
  const toSignalPercent = (odd: string) => {
    const value = Number.parseFloat(odd);
    if (!Number.isFinite(value) || value <= 0) return "--";
    return `${Math.min(99, Math.max(1, Math.round(100 / value)))}%`;
  };
    ...
    <div className="space-y-8 p-4 pb-20 sm:p-6 lg:p-8">
      <div className="space-y-4">
        <h1 className="text-3xl font-display font-bold tracking-tight text-white mb-1 flex items-center gap-3">
          <Activity className="w-8 h-8 text-primary" /> Market Signals Lab
        </h1>
        <p className="text-muted-foreground">
          Prototype probability boards for match, transfer, and manager storylines.
          Simulation only, not betting and not live market pricing.
        </p>
        <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/8 p-4">
          <div className="flex items-start gap-3">
            <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-yellow-500" />
            <p className="text-sm leading-6 text-muted-foreground">
              This page is a frontend concept surface. It should be read as a simulation-led
              signals lab, not as a sportsbook, betting board, or live market feed.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="-mx-4 flex gap-4 overflow-x-auto px-4 pb-6 hide-scrollbar sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
        {liveMatches.map((match, i) => (
          <div key={i} className="flex w-[220px] shrink-0 flex-col overflow-hidden rounded-xl border border-white/5 bg-card/40 glass-card transition-all duration-300 hover:border-primary/30 hover:shadow-[0_0_15px_rgba(118,255,3,0.15)] sm:w-[240px]">
      ...
      <Tabs defaultValue="transfers" className="w-full">
        <TabsList className="mb-6 flex h-auto w-full justify-start gap-2 overflow-x-auto rounded-xl border border-white/5 bg-card/50 p-1 hide-scrollbar">
          <TabsTrigger value="transfers" className="whitespace-nowrap">Transfer Signals</TabsTrigger>
          <TabsTrigger value="predictions" className="whitespace-nowrap">Scenario Boards</TabsTrigger>
          <TabsTrigger value="managers" className="whitespace-nowrap">Manager Pressure</TabsTrigger>
          <TabsTrigger value="matches" className="whitespace-nowrap">Match Probabilities</TabsTrigger>
        </TabsList>

        <TabsContent value="transfers" className="custom-scrollbar lg:max-h-[800px] lg:overflow-y-auto lg:pr-2">
           <Card className="glass-card bg-card/40 pb-12">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><TrendingUp className="w-5 h-5 text-primary" /> Summer Transfer Signals</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {fullTransferOdds.map((item, idx) => (
                  <div key={idx} className="flex flex-col items-start justify-between rounded-xl border border-border bg-background/50 p-4 sm:flex-row sm:items-center">
                    ...
                    <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
                      ...
                      <div className="rounded-lg bg-primary px-4 py-2 text-right text-primary-foreground">
                        <div className="text-[10px] uppercase tracking-[0.18em] opacity-75">signal</div>
                        <div className="font-bold text-lg">{toSignalPercent(item.odds)}</div>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <TabsContent value="predictions" className="custom-scrollbar lg:max-h-[800px] lg:overflow-y-auto lg:pr-2">
           <Card className="glass-card bg-card/40 pb-12">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><TrendingUp className="w-5 h-5 text-primary" /> Scenario Outlooks</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="managers" className="custom-scrollbar lg:max-h-[800px] lg:overflow-y-auto lg:pr-2">
           <Card className="glass-card bg-card/40 pb-12">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Users className="w-5 h-5 text-primary" /> Manager Pressure Board</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
        ...
        <TabsContent value="matches" className="space-y-6 custom-scrollbar lg:max-h-[800px] lg:overflow-y-auto lg:pr-2">
            ...
            {fullMatchOdds.map((match, idx) => (
              <Card key={idx} className="glass-card bg-card/40 hover:border-primary/30 transition-colors">
                <CardHeader className="pb-2">
                  ...
                  <CardTitle className="text-base font-display leading-tight sm:text-lg">{match.match.split(' vs ')[0]} <br/> <span className="text-sm text-muted-foreground">vs</span> <br/> {match.match.split(' vs ')[1]}</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-3 gap-2 mt-4">
                    <div className="bg-background border border-border rounded-lg p-2 text-center cursor-pointer hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all">
                      ...
                      <div className="font-bold">{toSignalPercent(match[1])}</div>
                    </div>
                    <div className="bg-background border border-border rounded-lg p-2 text-center cursor-pointer hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all">
                      <div className="text-[10px] text-muted-foreground mb-1">Draw</div>
                      <div className="font-bold">{toSignalPercent(match.X)}</div>
                    </div>
                    <div className="bg-background border border-border rounded-lg p-2 text-center cursor-pointer hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all">
                      <div className="text-[10px] text-muted-foreground mb-1">Away</div>
                      <div className="font-bold">{toSignalPercent(match[2])}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
